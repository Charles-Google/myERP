...
import {
  getReceivableEvaluationList,
  getReceivableEvaluationStatistics
} from "@/api/material";
import type { ReceivableEvaluationData } from "@/api/material";
...
// 表格数据
const tableData = ref<ReceivableEvaluationData[]>([]);
const total = ref(0);
...
const fetchTableData = async () => {
  try {
    console.log("开始获取应收评估表数据");
    const response = await getReceivableEvaluationList({
      page: currentPage.value,
      pageSize: pageSize.value,
      ...queryForm.value
    });
    console.log("应收评估表响应数据:", response);
...
const fetchStatistics = async () => {
  try {
    console.log("开始获取应收评估表统计数据");
    const response = await getReceivableEvaluationStatistics();
    console.log("应收评估表统计响应数据:", response);
    if (response.success && response.data) {
