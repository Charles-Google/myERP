...
import { getMaterialPurchaseList, getMaterialStatistics } from "@/api/material";
...
const fetchTableData = async () => {
  try {
    const { data } = await getMaterialPurchaseList({
    ...
    if (data) {
      ...
      totalMaterials.value = data.statistics.totalMaterials;
      totalAmount.value = data.statistics.totalAmount;
    }
  } catch (error) {
...
const fetchStatistics = async () => {
  try {
    const { data } = await getMaterialStatistics();
    if (data) {
  ...
  <div class="p-5 bg-white h-cal[calc(100vh-100px)] ">
    ...
    <div class="bg-white border-t border-gray-200">
      ...
      <el-table :data="tableData" class="w-full">
        ...
        <el-table-column prop="purchaseDate" label="销售日期" align="center" />
        <el-table-column prop="contractNo" label="合同编号" align="center" />
        <el-table-column prop="supplier" label="客户名称" align="center" />
        <el-table-column prop="material" label="销售物料名称" align="center" />
